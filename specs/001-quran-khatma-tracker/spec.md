...
**Created**: 2026-07-13  
**Status**: Draft  
**Input**: User description: "Mobile-first Arabic RTL single-page Quran reading tracker that helps a reader complete the Quran in 40 days with onboarding, daily cards, detail modal, progress tracking, reset, warm spiritual tone, and the provided 40-day reading allocation."
...
- Q: Should there be a today quick-access button? -> A: Yes; a prominent `اليوم: <رقم اليوم>` shortcut opens the suggested current day's detail modal directly.
- Q: Is start date needed? -> A: Store start date for future features but do not display it in the MVP.
- Q: Can the reader un-complete a day? -> A: Yes; tapping completion again asks `هل تريد إلغاء إنجاز هذا اليوم؟` and toggles the day back to incomplete on confirmation.
...
- Q: How should ayah ranges across surahs be displayed? -> A: Use `السورة <اسم السورة> الآية <رقم الآية> ← السورة <اسم السورة> الآية <رقم الآية>` with the arrow indicating reading direction.
- Q: Should progress sharing be included? -> A: No; sharing progress is out of scope for MVP.
- Q: What animations are allowed? -> A: Minimal modal fade-in, checkmark bounce, and progress bar fill only.
- Q: How should long surah names behave in small cards? -> A: Truncate after 15 characters with an ellipsis and show the full name in the detail modal.
...
5. **Given** the dashboard is visible, **When** the reader taps the `اليوم: <رقم اليوم>` shortcut, **Then** the suggested current day's detail modal opens directly.

---

### User Story 3 - Mark Daily Reading Complete (Priority: P1)

As a reader, I want to mark or unmark any day's portion from the card or detail view so my khatma progress reflects my real reading without strict enforcement.
...
1. **Given** any day card is visible, **When** the reader taps its card, **Then** a detail modal opens showing the day number, surah name or names, and ayah range from start to end.
2. **Given** the detail modal is open for an incomplete day, **When** the reader taps `✓ أَنْجَزْت`, **Then** the day is marked complete, the modal can close, the card shows completed state, and progress increases.
3. **Given** a completed day is shown, **When** the reader taps completion again and confirms `هل تريد إلغاء إنجاز هذا اليوم؟`, **Then** that day becomes incomplete and progress decreases.
...
- If the reader opens a completed day, the detail modal shows the completed status and does not duplicate completion records.
- If the reader closes the detail modal with `إغلاق`, no progress changes occur unless they explicitly marked the day complete.
...
- If a surah name is longer than the compact card can comfortably show, the compact card truncates it after 15 characters with an ellipsis and the detail modal shows the full name.
...
- **FR-010**: The tracker MUST provide a large suggested-current-day card treatment that includes a teal day header, `ورد اليوم`, `الآيات`, the surah name, the ayah range, and either `أنجزت` or a completed checkmark.
- **FR-011**: The tracker MUST provide a compact card treatment for non-current days that includes day number, surah name, ayah range, and a status indicator; surah names longer than 15 characters MUST truncate with an ellipsis.
...
- **FR-014**: Tapping any day card MUST open a day detail modal.
- **FR-015**: The day detail modal MUST show the day number, surah name or names, ayah range from start to end, a prominent `✓ أَنْجَزْت` action for completable days, and an `إغلاق` close action.
- **FR-015a**: Ayah ranges MUST be displayed as `السورة <اسم السورة> الآية <رقم الآية> ← السورة <اسم السورة> الآية <رقم الآية>`, with full surah names shown in the detail modal.
...
- **FR-016a**: Toggling a completed day back to incomplete MUST show the confirmation `هل تريد إلغاء إنجاز هذا اليوم؟` before changing progress.
...
- **FR-026**: The dashboard MUST include a prominent `اليوم: <رقم اليوم>` shortcut that opens the suggested current day's detail modal directly.
...
- **FR-029**: The tracker MUST use minimal animations only: modal fade-in, checkmark bounce, and progress bar fill.
...
- **SC-003**: A reader can identify and open the suggested current day's reading portion within 5 seconds of opening the dashboard.
...
- **SC-008**: In user review, at least 90% of Arabic-speaking reviewers can complete onboarding, mark a day complete, open a day detail, and reset progress without assistance.
...
- **SC-011**: Font loading avoids invisible text and causes no user-visible layout shift in primary dashboard and modal views.
